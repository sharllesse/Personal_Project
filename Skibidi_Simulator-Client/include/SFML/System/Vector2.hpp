...
public:
    ...
    Vector2<T> normalize(bool _normalize_it_self);

    Vector2<T> truncate(Vector2<T> _v, float _m, bool _truncate_it_self = false);

    Vector2<T> truncate(float _m, bool _truncate_it_self = false);
};
